...
{
public:
    COMPONENT_DEFINE(Predator, Animal)
        // declare methods to be called at the corresponding stages of the execution sequence
        COMPONENT_INIT(init);
    COMPONENT_UPDATE(update);

    int init();
    void predator_construct(Unigine::Math::vec3 start_position, string corpus_property);
    void update();
    void create_child();
    void die();
...
private:
    ...
    NodeDummyPtr predator_corpus_node;
    PredatorCorpus* predator_corpus;
    ...
    CallbackBase* new_predator_counter_callback;
